...
Each request requires an `x-api-key` header with your API key available after signing up. Find out more about [Authentication](/#authentication).
...
The API returns HTTP status codes and error codes. See this [page](/features/errors) for more details.
